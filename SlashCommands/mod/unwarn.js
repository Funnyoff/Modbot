            ...
            interaction.reply({ allowedMentions: { repliedUser: false }, embeds: [funnyembed], components: [bts] }).then(async lefunny => {
                ...
                collector.on("collect", async i => {
                    ...
                    if (i.customId === `yes.${random}`) {
                        ...
                        db.delete(`number.${message.guild.id}.${member.user.id}`)
                        db.delete(`info.${message.guild.id}.${member.user.id}`)

                        i.reply({ content: `${member.user.tag} à été Unwarn`, ephemeral: true })
                    }
