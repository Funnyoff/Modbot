...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if (db.get(`owners_${message.guild.id}_${message.user.id}`) === interaction.user.id) {
            ...
            if (member.roles.highest.position > client.user.id) return interaction.reply({ allowedMentions: { repliedUser: false }, content: `Ce role est au dessus de moi, je ne peux pas l'ajouter` })
            ...
            const funnyembed = new MessageEmbed()
                ...
                .setDescription(`**${interaction.user.tag}** êtes-vous sûr de vouloir ajouter à **${member.user.tag}** le rôle ${funnyrole}`)
            ...
            interaction.reply({ allowedMentions: { repliedUser: false }, embeds: [funnyembed], components: [bts] }).then(async lefunny => {
                ...
                collector.on("collect", async i => {
                    ...
                    if (i.customId === `yes.${random}`) {
                        ...
                        member.roles.add(funnyrole)
                        i.reply({ content: `Le role à bien été ajouté`, ephemeral: true })
                    }
